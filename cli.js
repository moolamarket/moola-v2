...
async function execute(network, action, ...params) {
  ...
  if (action == 'redeem') {
    ...
    console.log(
      'Redeem',
      (await lendingPool.methods.withdraw(reserve, amount, user).send({ from: user, gas: 2000000 }))
        .transactionHash
    );
    return;
  }
  if (action == 'delegate') {
    ...
    console.log(
      ...
      (await debtToken.methods.approveDelegation(to, amount).send({ from: user, gas: 2000000 }))
        .transactionHash
    );
    return;
  }
  ...
  if (action == 'migrate-step-2') {
    ...
    const reserveTokensMCEUR = await dataProvider.methods
      ...
      .call();
    const reserveTokensMCREAL = await dataProvider.methods
      .getReserveTokensAddresses(reserves.creal)
      .call();
    ...
    const debtTokenMCREAL = new eth.Contract(
      DebtToken,
      reserveTokensMCREAL.variableDebtTokenAddress
    );
