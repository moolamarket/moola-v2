...
makeSuite(
  'LendingPool liquidation - liquidator receiving aToken, part sent to reserve',
  (testEnv) => {
    const {
    ...
    before('Upgrade Collateral Manager with reserve percent', async () => {
    ...
    it('Liquidates the borrow', async () => {
      ...
      const expectedCollateralLiquidated = new BigNumber(principalPrice)
        ...
        .div(
          new BigNumber(collateralPrice).times(new BigNumber(10).pow(principalDecimals)).times(100)
        )
      ...
      expect(treasuryBalanceAfter.toString()).to.be.bignumber.almostEqual(
        new BigNumber(treasuryBalanceBefore.toString())
    ...
    it('User 3 deposits 1000 USDC, user 4 1 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
      const { users, pool, usdc, oracle, weth, aWETH, helpersContract } = testEnv;
