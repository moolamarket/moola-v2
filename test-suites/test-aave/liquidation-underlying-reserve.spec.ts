...
import { deployLendingPoolCollateralManagerWithReserve } from '../../helpers/contracts-deployments';
...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  const RESERVE_FEE_PERCENT = 2;

  before('Before LendingPool liquidation: set config', async () => {
    const newCollateralManager = await deployLendingPoolCollateralManagerWithReserve(false);
    await testEnv.addressesProvider.setLendingPoolCollateralManager(newCollateralManager.address);
  ...
  it('Liquidates the borrow', async () => {
    const { dai, weth, aWETH, users, pool, oracle, helpersContract } = testEnv;
    const liquidator = users[3];
    const borrower = users[1];
    const treasuryAddress = await aWETH.RESERVE_TREASURY_ADDRESS();
    const treasuryBalanceBefore = await aWETH.balanceOf(treasuryAddress);
    ...
    const treasuryBalanceAfter = await aWETH.balanceOf(treasuryAddress);
    ...
    const expectedCollateralLiquidated = new BigNumber(principalPrice.toString())
      .times(new BigNumber(amountToLiquidate).times(105 + RESERVE_FEE_PERCENT))
    ...
    const expectedCollateralForReserve = expectedCollateralLiquidated
      .times(RESERVE_FEE_PERCENT)
      .div(105 + RESERVE_FEE_PERCENT)
      .decimalPlaces(0, BigNumber.ROUND_DOWN);
    ...
    expect(treasuryBalanceAfter.toString()).to.be.bignumber.almostEqual(
      new BigNumber(treasuryBalanceBefore.toString()).plus(expectedCollateralForReserve).toFixed(0),
      'Invalid collateral on the treasury'
    );
    ...
    expect(ethReserveDataAfter.availableLiquidity.toString()).to.be.bignumber.almostEqual(
      new BigNumber(ethReserveDataBefore.availableLiquidity.toString())
        ...
        .plus(expectedCollateralForReserve)
        .toFixed(0),
  ...
  it('User 3 deposits 1000 USDC, user 4 1 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, weth, aWETH, helpersContract } = testEnv;
  ...
  it('User 4 deposits 10 AAVE - drops HF, liquidates the AAVE, which results on a lower amount being liquidated', async () => {
    const { aave, aAAVE, usdc, users, pool, oracle, helpersContract } = testEnv;

    const depositor = users[3];
    const borrower = users[4];
    const liquidator = users[5];
    const treasuryAddress = await aAAVE.RESERVE_TREASURY_ADDRESS();
    const treasuryBalanceBefore = await aAAVE.balanceOf(treasuryAddress);
    ...
    await oracle.setAssetPrice(
      usdc.address,
      new BigNumber(usdcPrice.toString()).multipliedBy(1.13).toFixed(0)
    );
    ...
    const treasuryBalanceAfter = await aAAVE.balanceOf(treasuryAddress);
    ...
    const expectedPrincipal = new BigNumber(collateralPrice.toString())
      ...
      .times(10000)
      .div(new BigNumber(liquidationBonus.toString()).plus(RESERVE_FEE_PERCENT * 100))
      .decimalPlaces(0, BigNumber.ROUND_DOWN);

    const expectedCollateralForReserve = expectedCollateralLiquidated
      .times(RESERVE_FEE_PERCENT * 100)
      .div(new BigNumber(liquidationBonus.toString()).plus(RESERVE_FEE_PERCENT * 100))
      .decimalPlaces(0, BigNumber.ROUND_DOWN);
